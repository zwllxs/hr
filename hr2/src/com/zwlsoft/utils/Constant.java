...
public class Constant {
	...
	private static ResourceBundle res = ResourceBundle.getBundle("conf/Appkey");
	//网站名称
	 ...
	 */
	public static String[] getPicCropSizes(Integer sysFlag)  {
		String corps = "";
		if(Constant.AlbumSysFlag.NOR_PHOTO.getValue().equals(sysFlag)){
			corps = res.getString("ssi.image.photo.crop.sizes");
		}
		if(Constant.AlbumSysFlag.SYS_ICON.getValue().equals(sysFlag)){
			corps = res.getString("ssi.image.icon.crop.sizes");
		}
		if(Constant.AlbumSysFlag.SYS_WBPIC.getValue().equals(sysFlag)){
			corps = res.getString("ssi.image.wbpic.crop.sizes");
		}
		if(Constant.AlbumSysFlag.SYS_VOIDE.getValue().equals(sysFlag)){
			corps = res.getString("ssi.image.vedio.crop.sizes");
		}
		if(org.apache.commons.lang.StringUtils.isNotBlank(corps)){
			return corps.split(",");
		}
		return new String[0];
	}
