                        ...
                        <tbody>
			                ...
			                <tr>
			                    <td width="150" class="e_label td_top"><i class="cc1">*</i>产品规格名称：</td>
			                     <td><label><input type="text" name="branchName" id="branchName" value="${productBranch.branchName!''}" required=true maxlength="50" class="w35"></label><span style="color:grey">填写项目的名称。</span></td>
			                 </tr>
			                 <tr>
...
<script>
  ...
  $("#save").bind("click",function(){
		 ...
		 if(refreshSensitiveWord($("input[type='text'],textarea"))){
			...
			$.ajax({
			url : "/vst_back/otherTicket/prod/prodbranch/updateProductBranch.do",
			type : "post",
			dataType:"json",
			async: false,
			...
			success : function(result) {
			   ...
			   if(result.code=="success"){
						$.alert(result.message,function(){
								location.href="/vst_back/otherTicket/prod/prodbranch/findProductBranchList.do?productId=${productBranch.productId!''}&categoryId=${categoryId!''}";
				   				updateDialog.close();
		   			});
					}else {
