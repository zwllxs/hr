            ...
            <tbody>
               ...
               <#---->	<tr>
                ...
                <tr <#if prodDestRe_index=0>name='no1'</#if>>
		            ...
		            <td>
		            	<input type="text" name="dest" id="dest${prodDestRe_index}" errorEle="descRecode" value="${prodDestRe.destName}" readonly = "readonly" required>
		            	<input type="hidden" name="prodDestReList[${prodDestRe_index}].destId" id="destId${prodDestRe_index}" value="${prodDestRe.destId}">
		            	<#if prodDestRe.mainFlag="Y">
		            		<label id="mainFlag">主目的地</label>
	            		<#else>
		            		<a class='btn btn_cc1' name='main_button'>设主目的地</a>
                ...
                <tr>
					...
					<td>
						<label>
							<select disabled="disabled" name="packageType" id="packageType" required>
                	 			<option value="">请选择</option>
			    				<#list packageTypeList as list>
			                    	<option value=${list.code!''} <#if prodProduct!=null && prodProduct.packageType==list.code>selected</#if> >${list.cnName!''}</option>
				                </#list>
				        	</select>
						</label>
						<input type="hidden" value="${prodProduct.packageType}" id="oldPackageType"/>
                    </td>
	            </tr> 
	           </table>
	           <table class="e_table form-inline">
	...
	$("#save").bind("click",function(){
		 ...
		 if(refreshSensitiveWord($("input[type='text'],textarea"))){
    		...
    		$.confirm(msg,function(){
				...
				refreshAddValue();
				$.ajax({
					url : "/vst_back/combTicket/prod/product/updateProduct.do",
					type : "post",
					dataType : 'json',
