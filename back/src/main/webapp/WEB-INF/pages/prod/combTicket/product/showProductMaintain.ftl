    ...
    <div class="pg_aside">
        <div class="aside_box">
            ...
            <ul class="pg_list J_list">
                ...
                <li class="active"><a target="iframeMain" href='javascript:void(0);' id="product">基本信息</a></li>
                <li class="cc1"><a target="iframeMain" href='javascript:void(0);' id="productBranch">产品规格</a></li>
                <li class="cc1"><a target="iframeMain" href='javascript:void(0);' id="prodPackage">组合商品</a></li>
...
<script>
	...
	$(document).ready(function(){
	 ...
	 var $LI = $(".J_list").find("li"), 
		...
		$("#productBranch").parent("li").click(function(){
			...
			//验证产品
			$.ajax({
					url : "/vst_back/combTicket/prod/prodbranch/showProductBranchCheck.do?productId="+productId,
					type : "post",
					async: false,
					dataType : 'json',
					success : function(result) {
						if(result.code == "error"){
							...
							$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
						 	$("#iframeMain").attr("src","/vst_back/combTicket/prod/prodbranch/findProductBranchList.do?productId="+productId+"&categoryId="+categoryId);
						}
					},
					error : function(result) {
						$.alert(result.message);
		...
		//组合商品
		$("#prodPackage").parent("li").click(function(){
			var productId = $("#productId").val();
			var categoryId = $("#categoryId").val();
			if(productId==""){
				$("#product").parent("li").eq(0).click();
			...
			$.ajax({
				...
				success : function(result) {
					if(result.code == "error"){
						...
						$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
					 	$("#iframeMain").attr("src","/vst_back/combTicket/prod/product/autonomy/showAutonomyProductMain.do?productId="+productId+"&categoryId="+categoryId);
					}
				},
				error : function(result) {
					$.alert(result.message);
	...
	function checkAndJump(){
		...
		//打开选择品类
		if(productId==""){
			$("#iframeMain").attr("src","/vst_back/combTicket/prod/product/showAddProduct.do?categoryId="+categoryId+"&timestamp="+Math.random(10));
		}else{
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		    $("#iframeMain").attr("src","/vst_back/combTicket/prod/product/showUpdateProduct.do?productId="+productId+"&timestamp="+Math.random(10));
		}
	}
	
	// 根据组合套餐类型设置左侧菜单
	function setCombTicketMenu(packageType){
		// 驴妈妈
		if('LVMAMA'==packageType){
			$('.f16',parent.document).eq(1).hide();
			$('.cc1',parent.document).hide();
			$('.cc1',parent.document).eq(1).show();
			$('.cc1',parent.document).eq(2).show();
		}else if('SUPPLIER'==packageType){
			// 供应商
			$('.f16',parent.document).eq(1).show();
			$('.cc1',parent.document).show();				
			$('.cc1',parent.document).eq(1).hide();				
		}	
	}	
