...
<script>
  ...
  $("#save").bind("click",function(){
			...
			$.ajax({
			url : "/vst_back/combTicket/prod/prodbranch/updateProductBranch.do",
			type : "post",
			dataType:"json",
			async: false,
			...
			success : function(result) {
			   ...
			   if(result.code=="success"){
						$.alert(result.message,function(){
								location.href="/vst_back/combTicket/prod/prodbranch/findProductBranchList.do?productId=${productBranch.productId!''}&categoryId=${categoryId!''}";
				   				updateDialog.close();
		   			});
					}else {
