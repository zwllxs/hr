...
var categorySelectDialog;
	$(document).ready(function(){
	 ...
	 var $LI = $(".J_list").find("li"), 
		...
		$("#productBranch").parent("li").click(function(){
			...
			//验证产品
			$.ajax({
					url : "/vst_back/visa/prodbranch/showProductBranchCheck.do?productId="+productId,
					type : "post",
					async: false,
					dataType : 'json',
					success : function(result) {
						if(result.code == "error"){
							...
							$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
						 	$("#iframeMain").attr("src","/vst_back/visa/prodbranch/findProductBranchList.do?productId="+productId+"&categoryId="+categoryId);
						}
					},
	...
	function checkAndJump(){
		...
		//打开选择品类
		if(productId==""){
			$("#iframeMain").attr("src","/vst_back/visa/product/showAddProduct.do?categoryId="+categoryId+"&timestamp="+Math.random(10));
		}else{
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		    $("#iframeMain").attr("src","/vst_back/visa/product/showUpdateProduct.do?productId="+productId+"&timestamp="+Math.random(10));
		}
	}
	...
	$("#suppGoods").parent("li").click(function(){
		if($("#productId").val()!=null && $("#productId").val()!=""){
			$.ajax({
				url : "/vst_back/visa/goods/goods/showSuppGoodsListCheck.do?productId="+$("#productId").val(),
				type : "post",
				async: false,
				dataType : 'json',
				success : function(result) {
					if(result.code == "error"){
						...
						$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
					 	$("#iframeMain").attr("src","/vst_back/visa/goods/goods/showSuppGoodsList.do?productId="+$("#productId").val());
					}
				},
				error : function(result) {
					$.alert(result.message);
	...
	$("#visaRange").parent("li").click(function(){
		...
		$("#iframeMain").attr("src","/vst_back/visa/range/findProdVisaRangeList.do?productId="+productId+"&categoryId="+categoryId);
