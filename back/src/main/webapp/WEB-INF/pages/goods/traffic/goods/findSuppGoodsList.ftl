...
<div class="iframe_content mt10">
        ...
        <form method="post" action='/vst_back/goods/traffic/findSuppGoodsList.do' id="searchForm">
...
<script>
		...
		$("#new_button").bind("click",function(){
			...
			if($("#supplierId").val()!=null && $("#supplierId").val()!=""){	
				new xDialog("/vst_back/goods/traffic/showAddSuppGoods.do",{productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractName:$("#contractName").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),contentManagerName:$("#contentManagerName").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"新增商品",width:800});
			}else{
				$.alert("请先选择供应商！");
			}	
		});
		...
		new xDialog("/vst_back/goods/traffic/showUpdateSuppGoods.do",{suppGoodsId:suppGoodsId,productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"编辑商品",width:800});
		});
		...
		$("a.cancelProp").bind("click",function(){
			...
			$.ajax({
				url : "/vst_back/goods/traffic/cancelProduct.do",
				type : "post",
				dataType:"JSON",
