...
<div class="iframe_content mt10">
        ...
        <form method="post" action='/vst_back/ship/goods/goods/findSuppGoodsList.do' id="searchForm">
...
<script>
		...
		if($("#supplierId").val()!=null && $("#supplierId").val()!=""){	
			new xDialog("/vst_back/ship/goods/goods/showAddSuppGoods.do",{productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractName:$("#contractName").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),contentManagerName:$("#contentManagerName").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"新增商品",width:800});
		}else{
			$.alert("请先选择供应商！");
		}	
		});
		...
		new xDialog("/vst_back/ship/goods/goods/showUpdateSuppGoods.do",{suppGoodsId:suppGoodsId,productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"编辑商品",width:800});
		});
		...
		$("a.cancelProp").bind("click",function(){
			...
			$.ajax({
				url : "/vst_back/ship/goods/goods/cancelProduct.do",
				type : "post",
				dataType:"JSON",
