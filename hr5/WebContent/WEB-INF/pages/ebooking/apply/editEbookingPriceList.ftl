...
<div class="iframe-content">   
	...
	<div class="iframe_search"> 
	<table class="s_table">
	<input type="radio" name="auditStatus" value="ACCESS_AUDIT" />审核通过
	<input type="radio" name="auditStatus" value="UNACCESS_AUDIT" />审核不通过
	<br></br>
	<label>提交审核结果备注：</label><textarea class="w35 textWidth"  style="height:80px" id="memo" name="memo"> </textarea></br></br>
		<a class="btn btn_cc1" id="confirm" data=${ebkProdApply.applyId!''}>提交审核结果</a>
	</table>
	</div>
</div>
<!-- //主要内容显示区域 -->
...
<script>
	$(function(){
		...
		$("#confirm").bind("click",function(){
			if(typeof($("input[name=auditStatus]:checked").val()) == "undefined"){
				$.alert("请选择是否审核通过！");
			}else{
				var entity = $(this);
				var applyId = entity.attr("data");
			 	$.confirm("确定提交审核结果？", function () {
					$.ajax({
						url : "/vst_back/ebooking/apply/editAuditStatus.do",
						type : "post",
						dataType:"JSON",
						data : {"applyId":applyId,"memo":$("#memo").val(),"applyType":'${ebkProdApply.applyType}',"auditStatus":$("input[name=auditStatus]:checked").val()},
						success : function(result) {
							if (result.code == "success") { 
								$.alert(result.message,function(){ 
								window.location = "/vst_back/ebooking/apply/findEbookingApplyList.do?auditStatus=PENDING_AUDIT"; 
								}); 
							}else { 
