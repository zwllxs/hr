...
<!--
        ...
        <form method="post" action='/vst_back/goods/shoreExcursions/findSuppGoodsList.do' id="searchForm">
...
<script>
		...
		if($("#supplierId").val()!=null && $("#supplierId").val()!=""){	
			new xDialog("/vst_back/goods/shoreExcursions/showAddSuppGoods.do",{productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractName:$("#contractName").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),contentManagerName:$("#contentManagerName").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"新增商品",width:800,ok:function(){
			//验证
			...
			$.ajax({
				url : "/vst_back/goods/shoreExcursions/addSuppGoods.do",
				type : "post",
		...
		new xDialog("/vst_back/goods/shoreExcursions/showUpdateSuppGoods.do",{suppGoodsId:suppGoodsId,productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"编辑商品",width:800,ok:function(){
			//验证
			if(!$("#dataForm").validate().form()){
				return false;
			}
			...
			$.ajax({
				url : "/vst_back/goods/shoreExcursions/updateSuppGoods.do",
				type : "post",
				data : $(".dialog #dataForm").serialize(),
				success : function(result) {
					confirmAndRefresh(result);
		...
		$("a.cancelProp").bind("click",function(){
			...
			$.ajax({
				url : "/vst_back/goods/shoreExcursions/cancelSuppGoods.do",
				type : "post",
				dataType:"JSON",
