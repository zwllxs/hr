...
<body>
    ...
    <!-- //工作区 -->
<div class="pg_main">
    <iframe id="iframeMain" name="iframeMain" src="" frameborder="0" style=" height:100%; background:#fff">
...
<script>
	...
	function checkAndJump(){
		...
		//打开选择品类
		if(productId==""){
			$("#iframeMain").attr("src","/vst_back/prod/shoreExcursions/showAddProduct.do");
		}else{
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		    $("#iframeMain").attr("src","/vst_back/prod/shoreExcursions/showUpdateProduct.do?productId="+productId+"&timestamp="+Math.random(10));
		}
	}
	...
	$("#suppGoods").parent("li").click(function(){
		...
		if($("#productId").val()!=null && $("#productId").val()!=""){
			$.ajax({
				url : "/vst_back/goods/shoreExcursions/showSuppGoodsListCheck.do?productId="+$("#productId").val(),
				type : "post",
				async: false,
				dataType : 'json',
				success : function(result) {
					if(result.code == "error"){
						...
						$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
					 	$("#iframeMain").attr("src","/vst_back/goods/shoreExcursions/showSuppGoodsList.do?productId="+$("#productId").val());
					}
				},
