...
<div class="iframe_content">
	...
	<form method="post" action='/vst_back/prod/shipAddition/findAdditionItemList.do' id="searchForm">
        <table class="s_table">
            <tbody>
                <tr>
...
$(function(){
	...
	$("#new_button").bind("click",function(){
		//打开弹出窗口
		window.open("/vst_back/prod/shipAddition/showProductMaintain.do?productId=");
		/**
		if($("#categoryId").val()!=null){	
		new xDialog("/vst_back/prod/shipAddition/showAddProduct.do",{categoryId:$('#categoryId').val()},{title:"新增产品",width:800,ok:function(){
			//验证
	...
	$("a.editProd").bind("click",function(){
		...
		window.open("/vst_back/prod/shipAddition/showProductMaintain.do?productId="+productId+"&categoryId="+categoryId+"&categoryName="+categoryName);
		return false;
	});
...
//设置为有效或无效
	$("a.cancelProd").bind("click",function(){
	 ...
	 $.confirm(msg, function () {
		$.ajax({
			url : "/vst_back/prod/shipAddition/cancelProduct.do",
			type : "post",
			dataType:"JSON",
