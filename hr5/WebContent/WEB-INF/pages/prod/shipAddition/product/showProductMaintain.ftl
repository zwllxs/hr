...
<script>
	...
	function checkAndJump(){
		...
		//打开选择品类
		if(productId==""){
			$("#iframeMain").attr("src","/vst_back/prod/shipAddition/showAddProduct.do");
		}else{
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		    $("#iframeMain").attr("src","/vst_back/prod/shipAddition/showUpdateProduct.do?productId="+productId+"&timestamp="+Math.random(10));
		}
	}
	...
	$("#suppGoods").parent("li").click(function(){
		...
		if($("#productId").val()!=null && $("#productId").val()!=""){
			$.ajax({
				url : "/vst_back/goods/shipAddition/showSuppGoodsListCheck.do?productId="+$("#productId").val(),
				type : "post",
				async: false,
				dataType : 'json',
				success : function(result) {
					if(result.code == "error"){
						...
						$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
					 	$("#iframeMain").attr("src","/vst_back/goods/shipAddition/showSuppGoodsList.do?productId="+$("#productId").val());
					}
				},
